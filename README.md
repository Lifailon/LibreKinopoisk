...
Вы также можете использовать **Kiwi** браузер, который доступен для установки из [Google Play](https://play.google.com/store/apps/details?id=com.kiwibrowser.browser&hl=en_US) или [GitHub](https://github.com/kiwibrowser/src.next/releases) репозитория.

> Браузер имеет открытый исходный код. По мимо установки любых расширений которые поддерживет компьютерная версия Google Chrome, из коробки доступно использование темного режима на всех сайтах, встроенную функцию блокировки рекламы, а также инструменты разработчика (DevTools).
